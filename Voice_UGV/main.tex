...
\section{Signal Processing — MFCC Feature Extraction}

MFCC extraction transforms raw audio into a compact, speaker-adapted
feature representation. The pipeline follows seven sequential stages.

\subsection{Framing Parameters}

Audio is segmented into overlapping frames:
\begin{itemize}
  \item Frame size: $N = 256$ samples (32\,ms at 8\,kHz)
  \item Hop size: $H = 128$ samples (16\,ms, 50\% overlap)
  \item Total frames for 1.5\,s: $(12000 - 256)/128 + 1 = 92$
\end{itemize}

\subsection{Pre-emphasis}

A first-order high-pass filter boosts high-frequency content:
\begin{equation}
  y[n] = x[n] - \alpha\, x[n-1], \quad \alpha = 0.97
  \label{eq:preemphasis}
\end{equation}
The signal is normalized to $[-1, 1]$ by dividing by 32768 prior to
windowing.
...
\begin{equation}
  ...
  \label{eq:powspec}
\end{equation}
where $X[k]$ denotes the FFT output. Only the first $N/2 + 1 = 129$
bins are retained.

\subsection{Mel Filterbank}

The Mel scale maps frequency to a perceptually uniform scale:
\begin{equation}
  m = 2595\log_{10}\!\left(1 + \frac{f}{700}\right)
  \label{eq:meltohz}
\end{equation}
Twenty-six triangular filters equally spaced on the Mel scale are
applied to the power spectrum. Log energy is computed per filter:
\begin{equation}
  E[m] = \log\!\left(\sum_{k} P[k]\,H_m[k]\right),
  \quad m = 0,\ldots,25
  \label{eq:melbank}
\end{equation}
where $H_m[k]$ is the $m$-th triangular filter. A floor of $10^{-10}$
prevents $\log(0)$.

\subsection{Discrete Cosine Transform}

DCT-II with orthonormal normalization decorrelates the 26 log-energies
into 13 MFCC coefficients:
\begin{equation}
...
Delta (velocity) features capture temporal dynamics and are appended
to each frame, doubling the feature dimension from 13 to 26:
\begin{equation}
  \Delta c[f][k] = \frac{c[f{+}1][k] - c[f{-}1][k]}{2}
  \label{eq:delta}
\end{equation}
Edge frames use the nearest valid neighbor.

\subsection{Cepstral Mean and Variance Normalisation (CMVN)}

CMVN removes channel effects and normalizes each coefficient across
all frames $F$:
\begin{align}
...
\section{Pattern Matching — Dynamic Time Warping}
...
DTW finds the optimal non-linear alignment. The local distance is the
Euclidean norm:
\begin{equation}
...
The accumulated cost matrix is filled by the recurrence:
\begin{equation}
...
A parallel path-length matrix $L$ tracks the number of steps taken
...
denotes the predecessor cell selected by the DTW recurrence.
\end{equation}

The normalized DTW score is:
\begin{equation}
  \text{score} = \frac{D[F_1{-}1][F_2{-}1]}{L[F_1{-}1][F_2{-}1]}
  \label{eq:dtwnorm}
\end{equation}

This normalization removes the bias introduced by different word
durations. Memory usage is reduced from $O(F_1 F_2)$ to $O(F_2)$
by maintaining only two rolling rows of $D$ and $L$.
...
minimum DTW score across all stored templates:
\begin{equation}
...
subject to an acceptance threshold $\tau$:
\begin{equation}
...
\section{User Adaptation and Tuning}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\columnwidth]{figs/image.png}
