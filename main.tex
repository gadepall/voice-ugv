...
\begin{table*}[t]
	...
	\begin{tabular}{@{}clp{5cm}p{8.5cm}@{}}
		\toprule
		\textbf{Stage} & \textbf{Step} & \textbf{Description} & \textbf{Equation} \\
		\midrule
		1 & Framing
		& 12{,}000 samples split into 92 overlapping frames; $N=256$ (32\,ms), $H=128$ (16\,ms).
		& --- \\
		2 & Pre-emphasis
		& First-order filter ($\alpha=0.97$) boosts high frequencies; signal normalised to $[-1,1]$.
		& $y[n] = x[n] - \alpha\,x[n-1]$ \\
		3 & Hamming Window
		& Per-frame multiplication suppresses spectral leakage.
		& $w[n] = 0.54 - 0.46\cos\!\left(\dfrac{2\pi n}{N-1}\right)$ \\
		4 & FFT + Power Spectrum
		& 256-point radix-2 FFT; one-sided power spectrum retains 129 bins.
		& $P[k] = \dfrac{|X[k]|^2}{N},\quad k=0,\ldots,\tfrac{N}{2}$ \\
		5 & Mel Filterbank
		& 26 triangular filters on Mel scale; log energy per filter; floor $10^{-10}$ prevents $\log(0)$.
		& $m = 2595\log_{10}\!\!\left(1+\dfrac{f}{700}\right)$;\quad $E[m]=\log\!\left(\sum_k P[k]H_m[k]\right)$ \\
		6 & DCT-II
		& Orthonormal DCT decorrelates 26 log-energies into 13 cepstral coefficients.
		& $c[k] = w_k\!\sum_{n=0}^{M-1}\!E[n]\cos\!\left(\dfrac{\pi k(2n+1)}{2M}\right)$ \\
		7 & Delta + CMVN
		& First-order differences expand vectors to 26-dim; CMVN normalises across all 92 frames.
		& $\Delta c[f][k]=\dfrac{c[f{+}1][k]-c[f{-}1][k]}{2}$;\quad $\hat{c}[f][k]=\dfrac{c[f][k]-\mu_k}{\sigma_k+\varepsilon}$ \\
		\bottomrule
	\end{tabular}
\end{table*}
	...
	\begin{table*}[t]
		...
		\label{tab:dtw}
		\renewcommand{\arraystretch}{1.5}
		\begin{tabular}{@{}clp{6cm}p{7cm}@{}}
			\toprule
			\textbf{Stage} & \textbf{Step} & \textbf{Description} & \textbf{Equation} \\
			\midrule
			1 & Local Distance
			& Euclidean distance between query frame $\mathbf{Q}[i]$ and template frame $\mathbf{T}[j]$.
			& $d(i,j) = \|\mathbf{Q}[i] - \mathbf{T}[j]\|_2$ \\
			2 & Cost Accumulation
			& Three-way recurrence fills $D$ left-to-right; only two rolling rows stored, reducing memory from $O(F_1 F_2)$ to $O(F_2)$.
			& $D[i][j] = d(i,j) + \min\bigl(D[i{-}1][j],\,D[i][j{-}1],\,D[i{-}1][j{-}1]\bigr)$ \\
			3 & Path Tracking
			& Matrix $L$ counts warping steps; normalised score removes duration bias between utterances.
			& $L[i][j] = 1 + L[p^*]$;\quad $\text{score} = \dfrac{D[F_1{-}1][F_2{-}1]}{L[F_1{-}1][F_2{-}1]}$ \\
			4 & Best-of-N
			& Command score is the minimum over up to $N=10$ templates; index~0 is the bundled template, indices 1--9 are user-recorded.
			& $s(\text{cmd}) = \min_{i=0}^{N-1}\,\text{score}(\mathbf{Q},\mathbf{T}_i)$ \\
			5 & Decision
			& Command with lowest score wins; accepted only if score $< \tau = 12.0$. See Table~\ref{tab:results} for observed ranges.
			& $\hat{c} = \underset{\text{cmd}}{\arg\min}\;s(\text{cmd}),\quad \min_{\text{cmd}}s(\text{cmd}) < \tau$ \\
			\bottomrule
		\end{tabular}
	\end{table*}
